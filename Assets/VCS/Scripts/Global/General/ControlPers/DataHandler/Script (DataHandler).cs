...
public class ControlPers_DataHandler : MonoBehaviour
{
    ...
    private const string SETTINGSDATA_FILE_NODE_AUDIO = "Audio";
    private const string SETTINGSDATA_FILE_NODE_SOUNDVALUE = "SoundValue";
    private const float SETTINGSDATA_FILE_NODE_SOUNDVALUE_DEFAULT = 0.5f;
    private const string SETTINGSDATA_FILE_NODE_MUSICVALUE = "MusicValue";
    private const float SETTINGSDATA_FILE_NODE_MUSICVALUE_DEFAULT = 0.5f;
    private XmlDocument SettingsData_Load()
    {
        ...
        {
            XmlNode _node_audio = _xmlDoc_settingsData.CreateElement(SETTINGSDATA_FILE_NODE_AUDIO); //Создаем раздел Audio для XML документа
            _xmlDoc_settingsData.AppendChild(_node_audio); //Записываем раздел Audio в XML документ

            XmlNode _node_soundValue = _xmlDoc_settingsData.CreateElement(SETTINGSDATA_FILE_NODE_SOUNDVALUE); //Создаем раздел SoundValue для XML документа
            _node_soundValue.InnerText = SETTINGSDATA_FILE_NODE_SOUNDVALUE_DEFAULT.ToString(); //Записываем значение в раздел SoundValue
            _node_audio.AppendChild(_node_soundValue); //Записываем раздел SoundValue в раздел Audio

            XmlNode _node_musicValue = _xmlDoc_settingsData.CreateElement(SETTINGSDATA_FILE_NODE_MUSICVALUE); //Создаем раздел MusicValue для XML документа
            _node_musicValue.InnerText = SETTINGSDATA_FILE_NODE_MUSICVALUE_DEFAULT.ToString(); //Записываем значение в раздел MusicValue
    ...
    private void Awake()
    {
        ...
        var _audio_soundValue_text = _xmlDoc_settingsData.SelectSingleNode(SETTINGSDATA_FILE_NODE_AUDIO + "/" + SETTINGSDATA_FILE_NODE_SOUNDVALUE).InnerText;
        settingsData.soundValue = float.Parse(_audio_soundValue_text);
        var _audio_musicValue_text = _xmlDoc_settingsData.SelectSingleNode(SETTINGSDATA_FILE_NODE_AUDIO + "/" + SETTINGSDATA_FILE_NODE_MUSICVALUE).InnerText;
    ...
    public void SaveSettings()
    {
        ...
        var _xmlDoc_settingsData = SettingsData_Load();
        _xmlDoc_settingsData.SelectSingleNode(SETTINGSDATA_FILE_NODE_AUDIO + "/" + SETTINGSDATA_FILE_NODE_SOUNDVALUE).InnerText = settingsData.soundValue.ToString(); //Перезаписываем значение в разделе SoundValue
        _xmlDoc_settingsData.SelectSingleNode(SETTINGSDATA_FILE_NODE_AUDIO + "/" + SETTINGSDATA_FILE_NODE_MUSICVALUE).InnerText = settingsData.musicValue.ToString(); //Перезаписываем значение в разделе MusicValue
